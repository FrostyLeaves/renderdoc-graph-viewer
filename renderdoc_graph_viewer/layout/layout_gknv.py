...
# Network-simplex pivot cap: a heuristic safety bound, NOT a termination proof.
# The loop normally exits earlier once no tree edge has a negative cut value
# (optimum reached); the cap only guards against pathological non-convergence.
_SIMPLEX_ITER_PER_EDGE = 4
_SIMPLEX_ITER_SLACK = 100
# ALAP tie-settling sweeps in the ranking phase (ties settle in <= 2 passes).
_BALANCE_ROUNDS = 2
...
class NetworkSimplex(object):
    ...
    def _pivot_find_enter(self, small_is_tail, et, eh):
        """Min-slack edge entering the smaller (tail) side: its tail lies
        outside that side, its head inside. Returns (enter_eid, slack), or
        (-1, None) if none exists (defensive)."""
        tail, head, rank, minlen = self.tail, self.head, self.rank, self.minlen
        tree, mark = self.tree, self.mark
        best = -1
        best_sl = None
        if small_is_tail:
            for eid in range(self.m):
    ...
    def _pivot_swap_tree_edge(self, leave, enter):
        """Structure swap: drop tree edge `leave`, add `enter` - tree flags,
        adjacency lists, and the tree_edges/tree_pos compaction."""
        tail, head = self.tail, self.head
        self.tree[leave] = False
        self.tree[enter] = True
        tadj = self.tree_adj
    ...
    def _pivot(self, leave):
        ...
        enter, best_sl = self._pivot_find_enter(small_is_tail, et, eh)
        if enter < 0:
        ...
        if root_in_tail:
            F_moved = -F_tail
            r0 = head[leave]      # moved side internal root
        ...
        self._pivot_swap_tree_edge(leave, enter)
    ...
    def solve(self, max_iter=None, balance_late=False, warm=None):
        ...
        self._init_tree()
        if max_iter is None:
            max_iter = _SIMPLEX_ITER_PER_EDGE * m + _SIMPLEX_ITER_SLACK
        start = 0
        it = 0
        cut = self.cut
        while it < max_iter:
...
def _weak_components(nodes, neighbors):
    """Flood-fill `nodes` into weakly-connected components. `neighbors[node]` is
    the symmetric adjacency (a list for int nodes, a dict for id nodes). Returns
    member lists in node-iteration order."""
    seen = set()
    comps = []
    for v in nodes:
        if v in seen:
...
def solve_ranks(n, edges, max_iter=None, balance_late=True):
    ...
    for (t, h, _ml, _w) in edges:
        if t != h:
            adj[t].append(h)
            adj[h].append(t)
    comps = _weak_components(range(n), adj)
...
_ORDER_ITER = 8
_TRANSPOSE_ROUNDS = 4   # adjacent-swap crossing-reduction passes per ordering iter
_WARM_SWEEPS = 4        # coordinate warm-start centering sweeps
_VIRTUAL_H = 1.0
...
def _compute_impl(ids, sizes, edges, rank_pairs, col_gap, node_gap,
    ...
    for (a, b) in union:
        nbr[a].append(b)
        nbr[b].append(a)
    comps = _weak_components(ids, nbr)
