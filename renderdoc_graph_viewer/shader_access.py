...
offline. Dispatched by ShaderEncoding; the caller (binding_scan) prepares the
...
# a typed cast that re-binds an initialised handle to a new var before use, e.g.
#   Handle __RWBuffer0_42 = (RWTypedBuffer<uint4>)_18;   then __RWBuffer0_42.Store(
# (UE5 emits this two-step form; the access is on the cast alias, not _18).
...
def parse(encoding, payload, rw_bindings):
    ...
    An index ABSENT from the result was not touched by the shader at all
    (distinct from UNKNOWN, which means touched but not statically
    attributable). Unsupported encoding / empty payload / no bindings -> {}
    (the caller then degrades every binding to staticallyUnused)."""
    ...
    try:
        ...
        return _all_unknown(rw_bindings)   # parser blew up -> conservative degrade
